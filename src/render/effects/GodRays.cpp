...
namespace {
struct PushConsts {
    float lightUV[2]; float density, decay, weight, farPlaneDepth; int32_t samples;
    float lightColorR, lightColorG, lightColorB;
};
}

VkShaderModule GodRays::loadShader(const std::string& path) {
    std::ifstream file(path, std::ios::ate | std::ios::binary);
    if (!file.is_open()) { fprintf(stderr, "GodRays: failed to open shader '%s'\n", path.c_str()); return VK_NULL_HANDLE; }
...
bool GodRays::Init(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool pool, VkQueue queue,
    ...
    fragShader_ = loadShader("shaders/godrays.frag.spv");
    ...
    vkutil::ClearColorToBlackAndTransition(device_, pool, queue, outputImage_); // valid+black even before this effect ever runs
    ...
    att.initialLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL; // matches the pre-clear transition above
...
void GodRays::RecordPass(VkCommandBuffer cmd, VkImageView depthView, float lightUvX, float lightUvY, bool lightVisible,
                          float lightColorR, float lightColorG, float lightColorB) {
    if (!outputView_) return;

    if (!enabled || !lightVisible) {
        // Clear to black so stale god rays don't remain visible on screen
    ...
    VkDescriptorImageInfo imgInfo{ sampler_, depthView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
    ...
    PushConsts pc{};
    pc.lightUV[0] = lightUvX; pc.lightUV[1] = lightUvY;
    pc.density = density; pc.decay = decay; pc.weight = weight;
    pc.farPlaneDepth = 500.0f; // matches SceneRenderer's far clip / clear-value sentinel
    pc.samples = std::clamp(samples, 1, kMaxSamples);
    pc.lightColorR = lightColorR; pc.lightColorG = lightColorG; pc.lightColorB = lightColorB;
...
void GodRays::Shutdown(VkDevice device) {
