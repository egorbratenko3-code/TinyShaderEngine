...
namespace {
struct PushConsts { float texelSize[2]; float focusDistance; float focusRange; float bokehStrength; float _pad[3]; };
}

VkShaderModule DepthOfField::loadShader(const std::string& path) {
    std::ifstream file(path, std::ios::ate | std::ios::binary);
    if (!file.is_open()) {
        fprintf(stderr, "DepthOfField: failed to open shader '%s' (compile via glslc, see Makefile)\n", path.c_str());
...
bool DepthOfField::Init(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool /*pool*/, VkQueue /*queue*/,
    ...
    fragShader_ = loadShader("shaders/dof.frag.spv");
    ...
    // ---- render pass: one color attachment, clear+overwrite each frame ----
    ...
    // ---- descriptor set layout: color (binding 0) + depth (binding 1) ----
    ...
    VkDescriptorPoolSize poolSize{ VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 4 }; // 2 bindings x 2 buffered sets
    ...
    // ---- pipeline: fullscreen triangle, no vertex buffer, no blending ----
...
void DepthOfField::RecordPass(VkCommandBuffer cmd, VkImageView colorView, VkImageView depthView) {
    if (!enabled || !outputView_) return;

    VkDescriptorSet set = inputSets_[frameSlot_];
    frameSlot_ = 1 - frameSlot_;

    VkDescriptorImageInfo colorInfo{ sampler_, colorView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
    VkDescriptorImageInfo depthInfo{ sampler_, depthView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
    VkWriteDescriptorSet writes[2]{};
    writes[0] = { VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, set, 0, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &colorInfo, nullptr, nullptr };
    writes[1] = { VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, set, 1, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &depthInfo, nullptr, nullptr };
    ...
    PushConsts pc{ {1.0f / width_, 1.0f / height_}, focusDistance, focusRange, bokehStrength, {0,0,0} };
...
void DepthOfField::Shutdown(VkDevice device) {
    ...
    framebuffer_ = VK_NULL_HANDLE; outputView_ = VK_NULL_HANDLE; outputImage_ = VK_NULL_HANDLE; outputMemory_ = VK_NULL_HANDLE;
    pipeline_ = VK_NULL_HANDLE; pipelineLayout_ = VK_NULL_HANDLE; setLayout_ = VK_NULL_HANDLE; descPool_ = VK_NULL_HANDLE;
    sampler_ = VK_NULL_HANDLE; renderPass_ = VK_NULL_HANDLE; vertShader_ = VK_NULL_HANDLE; fragShader_ = VK_NULL_HANDLE;
}
