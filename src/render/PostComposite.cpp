...
namespace { struct PushConsts { float bloomIntensity, godRaysIntensity; int32_t tonemapMode; float gamma; }; }


VkShaderModule PostComposite::loadShader(const std::string& path) {
    std::ifstream file(path, std::ios::ate | std::ios::binary);
    if (!file.is_open()) { fprintf(stderr, "PostComposite: failed to open shader '%s'\n", path.c_str()); return VK_NULL_HANDLE; }
...
bool PostComposite::Init(VkDevice device, VkPhysicalDevice physicalDevice, VkCommandPool /*pool*/, VkQueue /*queue*/,
    ...
    fragShader_ = loadShader("shaders/composite.frag.spv");
    ...
    VkDescriptorPoolSize poolSize{ VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 6 }; // 3 bindings x 2 buffered sets
...
void PostComposite::RecordPass(VkCommandBuffer cmd, VkImageView baseView, VkImageView bloomView, VkImageView godRaysView,
                                float bloomIntensity, float godRaysIntensity, int tonemapMode, float gamma) {
    ...
    VkDescriptorImageInfo infos[3] = {
        { sampler_, baseView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        { sampler_, bloomView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        { sampler_, godRaysView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
    };
    VkWriteDescriptorSet writes[3]{};
    for (int i = 0; i < 3; i++) {
        writes[i] = { VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, set, (uint32_t)i, 0, 1,
    ...
    PushConsts pc{ bloomIntensity, godRaysIntensity, tonemapMode, gamma };
...
void PostComposite::Shutdown(VkDevice device) {
